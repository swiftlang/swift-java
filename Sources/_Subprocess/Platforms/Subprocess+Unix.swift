...
@_implementationOnly import _SubprocessCShims
#else
...
extension Subprocess.Configuration {
  internal func preSpawn() throws -> (
    ...
    guard
      let executablePath = self.executable.resolveExecutablePath(
      ...
      throw CocoaError(
        ...
        userInfo: [
          "description": "\(self.executable.description) is not an executable"
        ]
      )
    }
    // Prepare arguments
    ...
    guard Self.pathAccessible(intendedWorkingDir.string, mode: F_OK) else {
      ...
      throw CocoaError(
        .fileNoSuchFile,
        userInfo: [
          "description": "Failed to set working directory to \(intendedWorkingDir)"
        ]
      )
    }
