...
extension JavaMethodMacro: BodyMacro {
  ...
  private static func isJNIGenericParameter(
    ...
    for contextNode in context.lexicalContext {
      guard let enclosingTypeName = contextNode.asProtocol(NamedDeclSyntax.self)?.name else {
        continue
      }
      ...
      if let decl = contextNode.asProtocol(WithGenericParametersSyntax.self) {
        if decl.genericParameterClause?.parameters.contains(where: {
          if $0.name.text == typeName {
            return true
          }
          /// Generic parameters generated by wrap-java follow this format: `<ClassName>_<GenericParameterName>`
          return $0.name.text == "\(enclosingTypeName)_\(typeName)"
        }) == true {
          return true
        }
      }
